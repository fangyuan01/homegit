...
public class StringUtil {
	...
	public static String javaBeanToString(Object object, Class clz) {
		String str = "JavaBean->" + clz.getName() + " ";
		Field[] fd = clz.getDeclaredFields();
		for (Field item : fd) {
			try {
				Field.setAccessible(fd, true); // 设置以访问私有属性
				str = str + " " + item.getName() + "->" + item.get(object);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static boolean isEquipmentSSID(String ssid) {
		return ssid.matches("[Gg][Zz][Jj][Aa][Dd][Ll]\\w*");
